...
- **多粒度精化链**：每个方法概念以 N 个 Inspiration 节点表示（N ≥ 1），`粒度` 整数字段（0 起，越小越抽象）区分层级，精化边（有向，低→高）串联。
...
**粒度**：整数，0 起递增（越小的数字越抽象）。0 级跨领域、1 级跨任务、2 级精确技术方案。绝大多数场景 0/1/2 三级足够。

**四种边**：

| 边 | 含义 | 方向 |
|---|---|---|
| 灵感精化边 | 低粒度→高粒度，同方法概念的纵向展开 | 有向 |
