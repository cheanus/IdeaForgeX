...
```bash
ifx batch-train [papers ...] [--file <path>]
```
...
| | `retrieve` | `inspect` |
|---|---|---|
| 节点字段 | 仅 `id` + `type` + `score` + `source` + `core_description`（瘦身） | 全字段展开（前提条件 / 操作步骤 / 问题类型 / 当前现状 / 未解决部分） |
...
# 训练阶段（运维）
$ ifx bootstrap          # 首次初始化
$ ifx train 1706.03762   # 训练 Attention Is All You Need
$ ifx train ViT           # 训练 ViT
$ ifx batch-train --file papers.txt  # 批量并行训练
$ ifx compact             # 图压缩合并重复节点
$ ifx stats               # 查看图谱规模
```
