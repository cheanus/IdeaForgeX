...
API_PRICE: dict[str, float] = {
    ...
    "patent_search": 0,
    "patent_info": 0,
    "paper_search_pro": 0.01,
    "paper_detail": 0.01,
    "patent_detail": 0.01,
    "org_detail": 0.01,
    "org_disambiguate": 0.01,
    "paper_qa_search": 0.05,
    "org_disambiguate_pro": 0.05,
    "paper_relation": 0.10,
    "org_paper_relation": 0.10,
    "org_patent_relation": 0.10,
    "venue_paper_relation": 0.10,
    "paper_list_by_keywords": 0.10,
    "venue_detail": 0.20,
    "paper_detail_by_condition": 0.20,
    "person_figure": 0.50,
...
def _request(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if attempt < MAX_RETRIES:
            ...
            print(
                f"[Retry] attempt={attempt}/{MAX_RETRIES} wait={backoff:.2f}s",
                file=sys.stderr,
            )
            time.sleep(backoff)
...
def paper_qa_search(
    ...
    topic_high: str = None,
    topic_middle: str = None,
    topic_low: str = None,
    title: list = None,
    doi: str = None,
    year: list = None,
    sci_flag: bool = False,
    n_citation_flag: bool = False,
    force_citation_sort: bool = False,
    force_year_sort: bool = False,
    author_terms: list = None,
    org_terms: list = None,
    author_id: list = None,
    org_id: list = None,
    venue_ids: list = None,
...
def paper_list_by_keywords(
    token: str, keywords: list, page: int = 0, size: int = 10
) -> Any:
...
def person_paper_relation(token: str, person_id: str) -> Any:
    ...
    return _request(
        token, "GET", "/api/person/paper/relation", params={"id": person_id}
    )
...
def person_patent_relation(token: str, person_id: str) -> Any:
    ...
    return _request(
        token, "GET", "/api/person/patent/relation", params={"id": person_id}
    )
...
def person_project(token: str, person_id: str) -> Any:
    ...
    return _request(
        token, "GET", "/api/project/person/v3/open", params={"id": person_id}
    )
...
def org_patent_relation(
    token: str, org_id: str, page: int = 1, page_size: int = 100
) -> Any:
...
def workflow_scholar_profile(token: str, name: str) -> dict:
    ...
    print(
        f"      Found: {scholar.get('name')} ({scholar.get('org')}), ID={person_id}",
        file=sys.stderr,
    )
...
def workflow_paper_deep_dive(
    token: str,
    title: str = None,
    keyword: str = None,
    author: str = None,
    order: str = "n_citation",
) -> dict:
    """
    ...
    if keyword or author:
        search_result = paper_search_pro(
            token, title=title, keyword=keyword, author=author, order=order, size=5
        )
        ...
        if not search_result or not search_result.get("data"):
            ...
            print(
                "      Title search returned no results; falling back to paper_search_pro...",
                file=sys.stderr,
            )
            search_result = paper_search_pro(
                token, title=title, keyword=title, author=author, order=order, size=5
            )
    ...
    if relation and relation.get("data"):
        ...
        cited_ids = [
            ...
            for c in all_cited[:20]
            if c.get("_id") or c.get("id")
        ]
        if cited_ids:
            print(
                f"[4/4] Batch-fetching basic info for {len(cited_ids)} cited papers...",
                file=sys.stderr,
            )
...
def workflow_org_analysis(token: str, org: str) -> dict:
    ...
    if not org_id:
        print(
            "      Disambiguation pro returned no ID; trying org search...",
            file=sys.stderr,
        )
...
def workflow_venue_papers(
    token: str, venue: str, year: Optional[int] = None, limit: int = 20
) -> dict:
    """
    ...
    print(
        f"[3/3] Fetching venue papers (year={year}, limit={limit})...", file=sys.stderr
    )
...
def workflow_paper_qa(
    token: str,
    query: str = None,
    topic_high: str = None,
    topic_middle: str = None,
    sci_flag: bool = False,
    sort_citation: bool = False,
    sort_year: bool = False,
    author_id: list = None,
    org_id: list = None,
    venue_ids: list = None,
    size: int = 10,
    ...
    print(
        f"[1/1] Academic Q&A search: query={query}, use_topic={use_topic}",
        file=sys.stderr,
    )
    qa_result = paper_qa_search(
        ...
        topic_middle=topic_middle,
        sci_flag=sci_flag,
        force_citation_sort=sort_citation,
    ...
    if query:
        print(
            "      paper_qa_search returned no results; falling back to paper_search_pro...",
            file=sys.stderr,
        )
...
def workflow_patent_search(
    token: str, query: str, page: int = 0, size: int = 10
) -> dict:
    """
    ...
    print(
        f"[2/2] Fetching details for {len(top_patents)} patents (parallel)...",
        file=sys.stderr,
    )
    details = []
    with ThreadPoolExecutor(max_workers=3) as pool:
        futs = {
            pool.submit(patent_detail, token, p["id"]): p["id"] for p in top_patents
        }
...
def workflow_scholar_patents(token: str, name: str) -> dict:
    ...
    print(
        f"[3/3] Fetching details for {len(top_patents)} patents (parallel)...",
        file=sys.stderr,
    )
    details = []
    with ThreadPoolExecutor(max_workers=3) as pool:
        futs = {
            pool.submit(patent_detail, token, p["patent_id"]): p["patent_id"]
...
Docs: https://open.aminer.cn/open/docs
    ...
    p.add_argument(
        "--query", help="Query string (natural language Q&A or patent search)"
    )
    ...
    p.add_argument(
        "--order",
        default="n_citation",
        choices=["n_citation", "year"],
        help="Sort order",
    )

    # Paper QA specific
    p.add_argument(
        "--topic_high", help="Required keyword array (JSON string; outer AND, inner OR)"
    )
    p.add_argument(
        "--topic_middle", help="Strongly boosted keywords (same format as topic_high)"
    )
    p.add_argument("--sci_flag", action="store_true", help="Return SCI papers only")
    p.add_argument(
        "--sort_citation", action="store_true", help="Sort by citation count"
    )
    p.add_argument(
        "--sort_year", action="store_true", help="Sort by year (most recent first)"
    )
    p.add_argument(
        "--author_id", help="Author ID filter; accepts single ID or JSON array string"
    )
    p.add_argument(
        "--org_id", help="Institution ID filter; accepts single ID or JSON array string"
    )
    p.add_argument(
        "--venue_ids", help="Conference/journal ID filter; accepts JSON array string"
    )
...
def main():
    ...
    if args.action == "scholar_profile":
        ...
        result = workflow_venue_papers(
            token, args.venue, year=args.year, limit=args.size
        )
        ...
        result = workflow_paper_qa(
            token,
            query=args.query,
            topic_high=args.topic_high,
            topic_middle=args.topic_middle,
            sci_flag=args.sci_flag,
            sort_citation=args.sort_citation,
            sort_year=args.sort_year,
            author_id=author_id_filter,
            org_id=org_id_filter,
            venue_ids=venue_ids_filter,
            size=args.size,
        )
        ...
        result = workflow_patent_search(
            token, args.query, page=args.page, size=args.size
        )
        ...
        if fn is None:
            ...
            parser.error(
                f"API function not found: {args.api}. Available APIs: {allowed}"
            )
